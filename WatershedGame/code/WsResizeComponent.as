package code {
  ...
  public class WsResizeComponent extends UIComponent implements IFocusManagerComponent {
		...
		protected var view:WsView;
		protected var closeBtn:Button;
		
		// The view is needed to fix the resizing issue when the scrollpane scrolled.
		public function WsResizeComponent(x:Number, y:Number, minWidth:Number, minHeight:Number,
																			wInitial:Number, hInitial:Number,
																			view:WsView) {
			super();
			this.xPos = x;
			this.yTop = y;
			...
			// iconWidth: 11.15; iconHeight: 11.1
			iconWidth = resizeIcon.width;
			iconHeight = resizeIcon.height;
			this.resizeIcon.x = xPos + wInitial - iconWidth;
			this.resizeIcon.y = yTop + hInitial - iconHeight;
			...
			closeBtn.label = WsConsts.CLOSE_NAME;
			closeBtn.setSize(WsConsts.CLOSE_BUTTON_WIDTH, WsConsts.CLOSE_BUTTON_HEIGHT);
			closeBtn.move(xPos + wInitial - WsConsts.CLOSE_BUTTON_WIDTH - 10, yTop + 10);
    ...
    // The child classes will implement the following functions if necessary;
		...
		protected function doDrag(e:MouseEvent):void {
			...
			closeBtn.x = xRight + iconWidth - WsConsts.CLOSE_BUTTON_WIDTH - 10;
			closeBtn.y = yTop + 10;
		}

		protected function drawHelperFrame(xRight:Number, yBottom:Number):void {
			frame.graphics.clear();
			frame.graphics.lineStyle(1, WsConsts.BLUE_COLOR, 1,
		...
		protected function clipX(x:Number):Number {
			var game:WatershedGame = root as WatershedGame;
      ...
      cursorShape.graphics.lineStyle(1);
			cursorShape.graphics.beginFill(WsConsts.WHITE_COLOR);
